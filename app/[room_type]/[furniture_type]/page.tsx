...
const FurniturePage = () => {
   ...
   const [furnitureSubData, setFurnitureSubData] = useState<any>(null);
      ...
      const fetchGroupData = async () => {
         try {
            const res = await fetch(
               `/api/rooms/${room_type}/${furniture_type}`
            );
            const data = await res.json();
            if (res.ok) {
               setFurnitureSubData(data.furnitureSubData);
               setFetching(false);
            } else {
   ...
   }, [room_type, furniture_type, furniture_sub]);
   ...
   const filteredProducts = (() => {
      const filteredProducts = furnitureSubData?.products.filter(
         (product: any) => {
            const materialMatch =
               selectedMaterial === 'All' ||
               (selectedMaterial === 'Leather' && product?.leather) ||
