...
import Filters from '~/app/[room_type]/[furniture_type]/[product]/components/filters/filters';
...
const TypeProducts = () => {
   ...
   useEffect(() => {
      const fetchGroupData = async () => {
         try {
            const res = await fetch(
               `/api/admin/${admin_furniture}/${group_products}`
            );
      ...
      window.addEventListener('groupUpdated', handleGroupCreated);

      return () => {
         window.removeEventListener('groupUpdated', handleGroupCreated);
      };
   }, [group_products, admin_furniture, type_products]);
   ...
   const filteredProducts = (() => {
      const filteredProducts = groupData?.products?.filter((product: any) => {
         const materialMatch =
            selectedMaterial === 'All' ||
            (selectedMaterial === 'Leather' && product?.leather) ||
      ...
      <main className="flex w-full flex-col     items-center   min-h-screen pb-8">
         <PageWrapper fetching={fetching} errorFetching={errorFetching}>
            ...
            >
               ...
               {groupData?.products?.length > 0 && <Filters {...filterProps} />}
