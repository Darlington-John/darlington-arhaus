...
const FurniturePage = () => {
   const [adminFunitureData, setAdminFunitureData] = useState<any>(null);
   const [fetching, setFetching] = useState(true);
   const [errorFetching, setErrorFetching] = useState(false);
   const { admin_furniture } = useParams();
   useEffect(() => {
      const fetchAdminFunitureData = async () => {
         try {
            const res = await fetch(`/api/admin/${admin_furniture}`);
            if (res.ok) {
               const data = await res.json();
               setAdminFunitureData(data.adminFurniture);
               setFetching(false);
            } else {
               setFetching(false);
      ...
      (async () => {
         await fetchAdminFunitureData();
      })();
      const handleGroupCreated = () => {
         fetchAdminFunitureData();
      };
      window.addEventListener('dataUpdated', handleGroupCreated);

      return () => {
         window.removeEventListener('dataUpdated', handleGroupCreated);
      };
   }, [admin_furniture]);
   const {
   ...
   const handleCreateGroup = async (e: any) => {
      ...
      formData.append('groupId', admin_furniture as any);
      await apiRequest({
            ...
            <section className="flex flex-col gap-4 px-5 py-10 xs:px-3 xs:py-5">
               <div className="flex items-center justify-between  w-full sm:flex-col sm:gap-2 sm:items-start">
                  <h1 className="neue-thin text-3xl capitalize text-darkGrey md:text-2xl  sm:text-xl">
                     Product groups In {adminFunitureData?.content}:
                  </h1>
               ...
               <div className="flex items-center  flex-wrap gap-5 ">
                  {adminFunitureData?.menu?.categories.length > 0 ? (
                     adminFunitureData?.menu.categories.map((data: any) => (
